...
async function inspectPage(page, slug) {
  ...
  const headerInfo = await page.evaluate(() => { const h = document.querySelector('.page-header'); if (!h) return { visible: false, height: 0 }; const r = h.getBoundingClientRect(); return { visible: r.width > 0, height: Math.round(r.height), overflowsViewport: r.right > window.innerWidth }; });
