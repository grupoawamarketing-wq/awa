define(['jquery', 'domReady!'], function ($) {
    ...
    function headerOverlayBlocksMenu() {
        ...
        let activeSearch = document.querySelector(
            ...
            '#search_autocomplete[aria-hidden="false"], ' +
        ...
        return !!(
            ...
            isVisible(activeMinicart) ||
            isVisible(activeSearch)
        );
    }

    function keepDesktopMenuExpanded() {
        /* A home atual inicia fechada por CSS; abertura permanente quebra o hero
           após a primeira interação com Departamentos. */
        return false;
    }
    ...
    function setOpenState($nav, open) {
        ...
        if (open && headerOverlayBlocksMenu()) {
            open = false;
            expanded = 'false';
        }
    ...
    function enforceDesktopHeaderNavVisualState($nav) {
        ...
        /* Home desktop: não force lista expandida; abertura fica por hover/clique. */
        if ($list.length && headerOverlayBlocksMenu()) {
