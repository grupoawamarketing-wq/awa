...
use GrupoAwamotos\B2B\Model\Registration\BackfillRazaoSocialFromCnpjCacheService;
...
class BackfillRazaoSocialFromCnpjCacheCommand extends Command
{
    public function __construct(
        private readonly BackfillRazaoSocialFromCnpjCacheService $backfillService,
    ...
    {
        $this->setName('b2b:registration:backfill-razao-social-from-cnpj-cache')
            ->setDescription('Preenche b2b_razao_social a partir do cache CNPJ (sem API externa)')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($apply && $limit === null) {
            $output->writeln('<error>Apply exige --limit para evitar execução global sem controle.</error>');
        ...
        $output->writeln('');
        $output->writeln('<info>Relatório backfill b2b_razao_social ← cache CNPJ</info>');
        ...
        $output->writeln(sprintf('Sem cache CNPJ: %d', $report['no_cache']));
        $output->writeln(sprintf('Cache sem razão válida: %d', $report['no_valid_razao_in_cache']));
