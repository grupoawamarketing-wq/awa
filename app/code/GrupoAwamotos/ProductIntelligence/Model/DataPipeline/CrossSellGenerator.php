...
class CrossSellGenerator
{
    ...
    public function generate(array $orders): int
    {
        ...
        if ($totalBaskets < 10) {
            $this->logger->warning('[ProductIntelligence Cross-sell] Too few baskets for analysis: ' . $totalBaskets);
            return 0;
        }

        $this->logger->info("[ProductIntelligence Cross-sell] Analyzing {$totalBaskets} baskets");
        ...
        $this->logger->info("[ProductIntelligence Cross-sell] Generated {$count} association rules");
        return $count;
    }
}
